...
export function parseFinancialCsv(text) {
  let cells;
  try { cells = parseCsvCells(text.replace(/^\uFEFF/, "")); } catch (error) { return { rows: [], errors: [error.message] }; }
  if (cells.length < 2) return { rows: [], errors: ["CSV 至少需要一行表头和一行数据"] };
  const headers = cells[0];
  if (new Set(headers.map(normalizeHeader)).size !== headers.length) return { rows: [], errors: ["CSV 表头重复"] };
  const rows = [];
  const errors = [];
  for (let index = 1; index < cells.length; index += 1) {
    ...
    for (const field of numericFields) {
      ...
      if (findHeader(headers, field) < 0) continue;
      const raw = valueFor(source, headers, field);
      if (raw === "") continue;
      record[field] = numberOrZero(raw);
      if (!Number.isFinite(record[field])) errors.push(`第 ${index + 1} 行 ${field} 不是有效数字`);
    }
    for (const key of ['companyId', 'sourceDocumentId']) {
      const raw = valueFor(source, headers, key);
      if (raw !== "" && (!Number.isSafeInteger(record[key]) || record[key] <= 0)) errors.push(`第 ${index + 1} 行 ${key} 无效`);
    }
    ...
    for (const key of ['companyId', 'sourceDocumentId']) if (!record[key]) delete record[key];
    for (const key of ['periodEnd', 'filingDate', 'currency', 'unitScale', 'auditNote', 'dataStatus']) if (findHeader(headers, key) < 0) delete record[key];
    if (source.length !== headers.length) errors.push(`第 ${index + 1} 行列数与表头不一致`);
    rows.push(record);
  }
  return { rows, errors };
}

export const FINANCIAL_IMPORT_EXAMPLE = "ticker,year,periodEnd,filingDate,currency,unitScale,dataStatus,revenue,grossProfit,freeCashFlow,roic,sourceDocumentId\nNVDA,2025,2025-01-26,2025-02-28,USD,millions,reported,130497,101467,58633,115,1";
