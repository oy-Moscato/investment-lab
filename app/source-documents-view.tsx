...
type Financial = {
  ...
  unitScale: string; dataStatus: string; sourceDocumentId: number | null; auditNote: string;
};
type SourceViewData = { companies: Company[]; sourceDocuments: SourceDocument[]; financials: Financial[] };
type Save = (payload: Record<string, unknown>, message?: string) => Promise<void>;
...
const currencies = ["USD", "EUR", "HKD", "CNY", "JPY", "GBP", "KRW", "其他"];
...
function TextField({ label, value, onChange, type = "text", required = false }: { label: string; value: string | number; onChange: (value: string) => void; type?: string; required?: boolean }) {
  return <label className="field"><span>{label}</span><input className="input" type={type} value={String(value ?? "")} required={required} onChange={(event) => onChange(event.target.value)} /></label>;
}
...
export default function SourceDocumentsView({ data, onSave }: { data: SourceViewData; onSave: Save }) {
  ...
  const submitSource = async (event: FormEvent) => { event.preventDefault(); if (!company) return; await onSave({ action: editingSourceId ? "update_source_document" : "create_source_document", id: editingSourceId ?? undefined, companyId: company.id, ...sourceForm }, editingSourceId ? "来源文档已更新" : "来源文档已创建"); setShowSourceEditor(false); setEditingSourceId(null); };
  const submitFinancial = async (event: FormEvent) => { event.preventDefault(); if (!company) return; await onSave({ action: editingFinancialId ? "update_financial" : "create_financial", id: editingFinancialId ?? undefined, companyId: company.id, ...financialForm }, editingFinancialId ? "财务年度已更新" : "财务年度已创建"); setShowFinancialEditor(false); setEditingFinancialId(null); };
